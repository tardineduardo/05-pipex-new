...
{
	if (sig == SIGALRM)
		ft_error_exit("Timeout reached", 1, STDERR_FILENO);
}
...
int	main(int argc, char *argv[], char *envp[])
{
	...
	head = NULL;
	signal(SIGALRM, handle_timeout);  // Set up timeout signal handler
	ft_validate_args(argc, argv);
	fill_commands(argc, argv, envp, &head);
	trav = head;
	while (trav != NULL)
{
		alarm(1);  // Set timeout for each child process (5 seconds in this example)
		ft_set_pipes_run_cmds(head, trav, argv, envp);
		alarm(0);  // Disable alarm after command completes
		trav = trav->next;
	}
